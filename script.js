...
const equationNames = [
  "4 APs",                                                         // Option 1
  "3 APs + ACT1",                                               // Option 2
  "3 APs + 2 Subjects",                                          // Option 3
  "3 APs + School (Grades 10, 11, 12)",                        // Option 4
  "2 APs + 4 Subjects",                                          // Option 5
  "2 APs + 2 Subjects + School (Grades 10, 11, 12)",           // Option 6
  "2 APs + ACT1 + School (Grades 10, 11, 12)",                 // Option 7
  "2 APs + 2 Subjects + ACT1",                                  // Option 8
  "1 AP + 6 Subjects",                                          // Option 9
  "1 AP + 2 Subjects + ACT1 + School (Grades 10, 11, 12)",     // Option 10
  "1 AP + 4 Subjects + ACT1",                                   // Option 11
  "1 AP + 4 Subjects + School (Grades 10, 11, 12)",            // Option 12
  "6 Subjects + School (Grades 10, 11, 12)",                   // Option 13
  "ACT1 + 4 Subjects + School (Grades 10, 11, 12)",            // Option 14
  "6 Subjects + ACT1",                                          // Option 15
  "8 Subjects"                                                  // Option 16
];
...
const ScoreData = {
    ...
    SAT1_OR_EST1: [
        { range: "1050-1060", equivalency: 75 },
        { range: "1070-1080", equivalency: 76 },
        { range: "1090-1100", equivalency: 77 },
        { range: "1110-1120", equivalency: 78 },
        { range: "1130-1140", equivalency: 79 },
        { range: "1150-1160", equivalency: 80 },
        { range: "1170-1180", equivalency: 81 },
        { range: "1190-1200", equivalency: 82 },
        { range: "1210-1220", equivalency: 83 },
        { range: "1230-1240", equivalency: 84 },
        { range: "1250-1260", equivalency: 85 },
        { range: "1270-1280", equivalency: 86 },
        { range: "1290-1300", equivalency: 87 },
        { range: "1310-1320", equivalency: 88 },
        { range: "1330-1340", equivalency: 89 },
        { range: "1350-1360", equivalency: 90 },
        { range: "1370-1380", equivalency: 91 },
        { range: "1390-1400", equivalency: 92 },
        { range: "1410-1420", equivalency: 93 },
        { range: "1430-1440", equivalency: 94 },
        { range: "1450-1460", equivalency: 95 },
        { range: "1470-1480", equivalency: 96 },
        { range: "1490-1500", equivalency: 97 },
        { range: "1510-1520", equivalency: 98 },
        { range: "1530-1540", equivalency: 99 },
        { range: "1550-1600", equivalency: 100 }
    ],
...
// Reordered to match the equationNames array
const Equations = [
    // Equation 0: 4 APs
    (AP1, AP2, AP3, AP4) => {
    ...
    // Equation 1: 3 APs + ACT1
    (AP1, AP2, AP3, ACT1) => {
        return ((AP1 + AP2 + AP3) / 3) * 0.75 + (ACT1 * 0.25);
    },
    
    // Equation 2: 3 APs + 2 Subjects
    (AP1, AP2, AP3, Subject1, Subject2) => {
        return ((AP1 + AP2 + AP3) / 3) * 0.75 + ((Subject1 + Subject2) / 2) * 0.25;
    },
    
    // Equation 3: 3 APs + School (Grades 10, 11, 12)
    (AP1, AP2, AP3, Grade10GPA, Grade11GPA, Grade12GPA) => {
        return ((AP1 + AP2 + AP3) / 3) * 0.75 + ((Grade10GPA + Grade11GPA + Grade12GPA) / 3) * 0.25;
    },
    
    // Equation 4: 2 APs + 4 Subjects
    (AP1, AP2, Subject1, Subject2, Subject3, Subject4) => {
        return ((AP1 + AP2) / 2) * 0.5 + ((Subject1 + Subject2 + Subject3 + Subject4) / 4) * 0.5;
    },
    
    // Equation 5: 2 APs + 2 Subjects + School (Grades 10, 11, 12)
    ...
    // Equation 6: 2 APs + ACT1 + School (Grades 10, 11, 12)
    (AP1, AP2, ACT1, Grade10GPA, Grade11GPA, Grade12GPA) => {
        return ((AP1 + AP2) / 2) * 0.5 + (ACT1 * 0.25) + ((Grade10GPA + Grade11GPA + Grade12GPA) / 3) * 0.25;
    },
    
    // Equation 7: 2 APs + 2 Subjects + ACT1
    (AP1, AP2, Subject1, Subject2, ACT1) => {
        return ((AP1 + AP2) / 2) * 0.5 + ((Subject1 + Subject2) / 2) * 0.25 + (ACT1 * 0.25);
    },
    
    // Equation 8: 1 AP + 6 Subjects
    ...
    // Equation 9: 1 AP + 2 Subjects + ACT1 + School (Grades 10, 11, 12)
    ...
    // Equation 10: 1 AP + 4 Subjects + ACT1
    (AP1, Subject1, Subject2, Subject3, Subject4, ACT1) => {
        return (AP1 * 0.25) + ((Subject1 + Subject2 + Subject3 + Subject4) / 4) * 0.5 + (ACT1 * 0.25);
    },
    
    // Equation 11: 1 AP + 4 Subjects + School (Grades 10, 11, 12)
    ...
    // Equation 12: 6 Subjects + School (Grades 10, 11, 12)
    ...
    // Equation 13: ACT1 + 4 Subjects + School (Grades 10, 11, 12)
    ...
    // Equation 14: 6 Subjects + ACT1
    (Subject1, Subject2, Subject3, Subject4, Subject5, Subject6, ACT1) => {
        return ((Subject1 + Subject2 + Subject3 + Subject4 + Subject5 + Subject6) / 6) * 0.75 + (ACT1 * 0.25);
    },
    
    // Equation 15: 8 Subjects
...
// Helper function for scrolling to top with enhanced mobile support
function scrollToTop() {
    // Check if we're in a Wix iframe
    const isWixIframe = detectIframe() && window.self !== window.top;
    
    // Function to scroll the current window with multiple fallbacks
    const scrollCurrentWindow = () => {
        // Small delay to ensure DOM is ready
        setTimeout(() => {
            // Try different scrolling methods with fallbacks
            const scrollOptions = {
                top: 0,
                left: 0,
                behavior: 'smooth'
            ...
            scrollElements.forEach(scroll => {
                try { scroll(); } catch(e) { /* Ignore errors */ }
            });
    ...
    // Function to scroll parent window if possible
    const scrollParentWindow = () => {
        if (!isWixIframe) return;
        
        try {
            // Post a message to the parent window to request scrolling
            window.parent.postMessage({ 
                type: 'SCROLL_TOP',
                source: 'equivalency-calculator' 
            }, '*');
            
            // Also try direct scrolling if same origin
    ...
    // Scroll both current and parent windows
    scrollCurrentWindow();
    scrollParentWindow();
    
    // Add a small delay and try again for mobile devices
    setTimeout(() => {
        scrollCurrentWindow();
        scrollParentWindow();
    }, 300);
...
// Function to detect if we're in a Wix iframe
const detectIframe = () => {
    try {
        // Check if we're in any iframe
        const inIframe = window.self !== window.top;
        if (!inIframe) return false;
        
        // Additional check for Wix-specific environment
        try {
            // Check for Wix environment variables
            if (window.parent && window.parent.wixBiSession) {
                return true;
            }
            
            // Check for Wix iframe class
            if (window.frameElement && window.frameElement.classList.contains('wix-iframe')) {
                return true;
            }
            
            // Check for Wix in URL
            if (window.location !== window.parent.location && 
                (window.parent.location.href.includes('wix.com') || 
                 window.parent.document.referrer.includes('wix.com'))) {
                return true;
            }
            
            return inIframe;
        } catch (e) {
            // If we can't access parent properties, we're likely in a cross-origin iframe
            return true;
        }
    } catch (e) {
...
// Apply iframe-specific styling and setup message listeners
const applyIframeStyles = () => {
    ...
    if (isInIframe) {
        ...
        // Add a small delay to ensure Wix has finished loading
        setTimeout(() => {
            // Try to adjust the iframe height to fit content
            try {
                if (window.parent && window.parent !== window) {
    ...
    window.addEventListener('message', (event) => {
        // Verify the message is from a trusted source if needed
        ...
        if (event.data && event.data.type === 'SCROLL_TO') {
            try {
                ...
                window.scrollTo(0, event.data.top || 0);
            }
        }
    });
};
...
function createInputField(param, type) {
    ...
    if (type === 'AP') {
        ...
        ScoreData.AP.forEach(item => {
            ...
            option.setAttribute('data-equivalency', item.equivalency);
        ...
        if (select.options.length > 0) {
            ...
            select.setAttribute('data-equivalency', ScoreData.AP[0].equivalency);
        }
        
        // Update data-equivalency when selection changes
        select.addEventListener('change', function() {
            const selectedOption = this.options[this.selectedIndex];
            this.setAttribute('data-equivalency', selectedOption.getAttribute('data-equivalency'));
        });
        ...
        const types = ['ACT', 'EST', 'SAT', 'Arabic', 'MOE (تاريخ الأردن - التربية الإسلامية)'];
        const displayNames = ['ACT Subject / ACT 1', 'EST/SAT SUBJECT', 'SAT1', 'Arabic', 'MOE (تاريخ الأردن - التربية الإسلامية)'];
...
function updateScoreInput(param, type, container) {
    ...
    if (type.startsWith('MOE')) {
        ...
        input.setAttribute('data-equivalency', '50');
        
        // Update data-equivalency when input changes
        input.addEventListener('input', function() {
            this.setAttribute('data-equivalency', this.value);
        });
        ...
        arabicGrades.forEach(item => {
            ...
            // Set initial data-equivalency on the select
            if (select.selectedIndex === 0) {
                select.setAttribute('data-equivalency', item.equivalency);
            }
        });
        ...
        // For ACT, use the ACT1 scores (16-36) with their equivalency values from ScoreData.ACT1
        const select = document.createElement('select');
        select.id = param;
        select.name = param;
        select.className = 'form-select';
        
        // Get the ACT1 scores from ScoreData and sort them in descending order
        const actScores = ScoreData.ACT1.sort((a, b) => b.score - a.score);
        ...
        actScores.forEach(item => {
            ...
            // Also set the data-equivalency on the select itself for the current value
            if (select.selectedIndex === 0) {
                select.setAttribute('data-equivalency', item.equivalency);
            }
        });
        
        // Add change event to update the data-equivalency attribute when selection changes
        select.addEventListener('change', function() {
            const selectedOption = this.options[this.selectedIndex];
            this.setAttribute('data-equivalency', selectedOption.getAttribute('data-equivalency'));
        });
        ...
        if (select.options.length > 0) {
            select.selectedIndex = 0;
            select.setAttribute('data-equivalency', satRanges[0].equivalency);
        }
        ...
        if (select.options.length > 0) {
            select.selectedIndex = 0;
            select.setAttribute('data-equivalency', estRanges[0].equivalency);
        }
...
function handleCalculate() {
    ...
    // Always scroll to top on both mobile and desktop
    // Use our enhanced scrollToTop function which handles both regular and iframe contexts
    scrollToTop();
    
    // For Wix iframes, also send a message to ensure the parent scrolls to top
    if (detectIframe()) {
        try {
            window.parent.postMessage({
                type: 'SCROLL_TOP',
            ...
            // Ignore errors if we can't access parent
        }
    }
}
...
function calculateResults() {
    ...
    // Get values for each parameter using the displayed equivalency values
    const values = [];
    const displayValues = [];
    ...
    params.forEach(param => {
        ...
        if (element) {
            // Get the equivalency value from the data-equivalency attribute
            const equivalencyValue = element.getAttribute('data-equivalency');
            if (equivalencyValue) {
                const value = parseFloat(equivalencyValue);
                values.push(value);
                displayValues.push({
                    name: param,
                    value: value
                ...
                // Fallback to direct value if no equivalency is set
