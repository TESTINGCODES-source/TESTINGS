...
const ScoreData = {
    ...
    // Legacy ACT format for backward compatibility
    ACT: {
...
function createInputField(param, type) {
    ...
    if (type === 'AP') {
        ...
        labelText = `Subject ${subjectNumber} (ACT1/SAT1_OR_EST1/SAT/Arabic)`;
    } else if (type === 'Grade') {
    ...
    if (type === 'AP') {
        ...
        // AP scores are 2-5 based on the new data structure
        ...
        // Updated test types based on the new data structure
        const types = ['ACT1', 'SAT1_OR_EST1', 'SAT', 'Arabic', 'MOE'];
        const displayNames = ['ACT1', 'SAT1/EST1', 'SAT', 'Arabic', 'MOE'];
        ...
        // Initial score input based on default type (ACT1)
        updateScoreInput(param, 'ACT1', scoreContainer);
...
function updateScoreInput(param, type, container) {
    ...
    if (type === 'MOE') {
        ...
        // For ACT1, use the new array structure
        const select = document.createElement('select');
        select.id = param;
        select.name = param;
        select.className = 'form-select';
        
        // Use ACT1 array from ScoreData
        ScoreData.ACT1.forEach(item => {
            const option = document.createElement('option');
            option.value = item.equivalency;
            option.textContent = item.score;
        ...
        // For SAT, use the new range-based structure
        const select = document.createElement('select');
        select.id = param;
        select.name = param;
        select.className = 'form-select';
        
        // Use SAT array from ScoreData
        ...
        container.appendChild(select);
    } else if (type === 'SAT1_OR_EST1') {
        // For SAT1/EST1, use the new range-based structure
        const select = document.createElement('select');
        select.id = param;
        select.name = param;
        select.className = 'form-select';
        
        // Use SAT1_OR_EST1 array from ScoreData
