...
function handleCalculate() {
    ...
    // Direct brute-force approach for mobile scrolling
    // First, try immediate scroll
    window.scrollTo(0, 0);
    
    // Then try with increasing delays
    var scrollDelays = [10, 50, 100, 300, 600, 1000];
    
    // Function to try all possible scroll methods
    function tryAllScrollMethods() {
        // Standard methods
        window.scrollTo(0, 0);
        document.body.scrollTop = 0;
        document.documentElement.scrollTop = 0;
        ...
        // Try to scroll parent if in iframe
        try {
            if (window.parent && window.parent !== window) {
                window.parent.scrollTo(0, 0);
            }
    ...
    // Apply all scroll methods with increasing delays
    scrollDelays.forEach(function(delay) {
        setTimeout(tryAllScrollMethods, delay);
    });
}
