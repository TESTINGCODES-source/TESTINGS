...
function handleCalculate() {
    ...
    // Enhanced scrolling for Wix and other embeds - with improved mobile support
    scrollToTop();
    
    // Additional scrolling attempts with delays for Wix embedding
    // This helps ensure scrolling works on both mobile and desktop when embedded
    setTimeout(() => {
        scrollToTop();
        
        // Try to scroll the element containing the results directly
        resultsSection.scrollIntoView({ behavior: 'smooth', block: 'start' });
        
        // Enhanced mobile scrolling for Wix embedding
        try {
            // For Wix specific - try to scroll the iframe container
            if (window.parent && window.parent !== window) {
                // Try to send a message to the parent window to scroll
                window.parent.postMessage({ 
                    type: 'scroll-to-results',
                    isMobile: isMobile,
                    timestamp: Date.now()
                }, '*');
                ...
                try {
                    ...
                    for (let i = 0; i < iframes.length; i++) {
                        if (iframes[i].contentWindow === window) {
                            ...
                            if (isMobile) {
                                // Get the position of the results section relative to the viewport
                                const rect = resultsSection.getBoundingClientRect();
                                const scrollTop = window.pageYOffset || document.documentElement.scrollTop;
                                const targetY = rect.top + scrollTop;
                                
                                // Scroll the parent window to the results section
                                window.parent.scrollTo({
                                    top: targetY,
                                    behavior: 'smooth'
                                });
                                
                                // Try to scroll all parent elements in Wix
