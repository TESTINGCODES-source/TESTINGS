...
function handleCalculate() {
    ...
    // Enhanced scrolling for Wix and other embeds
    scrollToTop();
    ...
    setTimeout(() => {
        ...
        try {
            ...
            // Try to find and scroll Wix containers
            if (window.parent && window.parent !== window) {
                ...
                for (let i = 0; i < iframes.length; i++) {
                    if (iframes[i].contentWindow === window) {
                        ...
                        // For mobile, also try to scroll all parent elements
                        if (isMobile) {
    ...
    // Mobile-specific additional scrolling attempts with longer delays
    if (isMobile) {
        const mobileDelays = [500, 1000, 1500, 2000];
...
function displayResults(results) {
    ...
    results.parameters.forEach(param => {
        ...
        item.textContent = `${formattedName}: ${param.value}%`;
