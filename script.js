...
function handleCalculate() {
    ...
    // Simple automatic scroll to top - non-lagging approach
    window.scrollTo({
        top: 0,
        behavior: 'auto' // Using 'auto' instead of 'smooth' to avoid lag
    });
    
    // For Wix embedding - use multiple attempts with increasing delays
    // This helps ensure scrolling works without causing lag
    [0, 50, 100, 300, 500].forEach(function(delay) {
        setTimeout(function() {
            // Basic scroll
            window.scrollTo(0, 0);
            document.body.scrollTop = 0;
            ...
            // Try to notify parent frame (Wix) to scroll as well
            try {
                if (window.self !== window.top) {
                    window.parent.postMessage({
                        type: 'calculator-scroll',
                        action: 'top',
                        timestamp: Date.now()
                    }, '*');
                }
